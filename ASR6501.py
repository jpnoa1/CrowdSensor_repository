...
class asr6501:
    ...
    def checkForDownlink(self):
        ...
        if rx.startswith("OK+RECV:"):
            self.logger.debug(rx)
            payload_part = rx[8:]  # tudo depois de "OK+RECV:"
            # split no máximo em 4 partes: TYPE, PORT, LEN e RESTO (DATA)
            parts = payload_part.split(",", 3)

            # unpack em tres partes
            mType = parts[0]
            port  = parts[1] if len(parts) > 1 else ""
            msglen= parts[2] if len(parts) > 2 else "0"
            msg   = parts[3] if len(parts) > 3 else ""

            if self.downlinkCallback:
                self.downlinkCallback(mType, port, msglen, msg)
